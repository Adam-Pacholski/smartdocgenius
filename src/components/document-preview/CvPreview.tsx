...
const CvPreview: React.FC<CvPreviewProps> = ({ 
  ...
  const handleExportPdf = async () => {
    ...
    const fileName = `cv_${formData.firstName || 'dokument'}_${formData.lastName || ''}_${new Date().toISOString().slice(0, 10)}.pdf`;
    ...
    try {
      ...
      toast({
        title: "Eksport PDF",
        description: "Dokument CV został wygenerowany i pobrany.",
      });
    } catch (error) {
    ...
    <Card className="shadow-subtle h-full">
      <CardHeader className="pb-2 flex flex-row items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg">Podgląd CV</CardTitle>
          {pageCount > 1 && (
